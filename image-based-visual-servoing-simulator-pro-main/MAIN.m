...
% 5 = DD-MPC + Sequential Lambda IBVS (data-driven MPC with lambda-shaped references)
control_method = 5;  
...
KDDMPC = struct('Ni',10,'Np',4,'lambda_g',1e-3, ...
                  'lambda_min',0.5,'lambda_max',3.0, ...
                  'Wy',1.0,'Wu',0.05,'sat_per_axis',true, ...
                  'min_data',24,'Wmax',200, ...
                  'alpha_c',5.0,'beta_c',5.0, ...
                  'normalize',true,'use_lambda_search',true, ...
                  'blend_ibvs',0.3,'nbases',6,'nslopes',5);
...
elseif control_method == 5
    %% Data-Driven MPC + Sequential Lambda reference shaping
    disp(['执行DD-MPC + Sequential Lambda 步骤 ', num2str(t)]);
    [control]=DDMPC_IBVS(dt, qd, q(:,t+1), control, ubound, wbound, lamda, z, umax, vmax, KDDMPC, t);
